...
export function transact(number, amount, type, setUsers = null) {
  let multiplier = 1;
  // quando o usuário solicitar credito
  if (type === "add" || type === "credit") multiplier = 1;
  // quando o usuário solicitar debito
  ...
  for (const user of users) {
    if (user.number === number) {
      ...
      // verifica se o usuario solicitou credito
      ...
      // verifica se o usuario solicitou debito
...
export function getDateToday() {
  ...
  return `${transDate.toLocaleString("en-us", {
    month: "long",
  })} ${transDate.getDate()}, ${transDate.getFullYear()}`;
}
