    ...
    return (
      <div className={`budget-row ${style}`}>
        <div className="budget-title">
          <button type="button" onClick={() => deleteRow(index)} className="mr">
            <i class="bx bxs-x-square"></i>
          </button>
          <button type="button" onClick={() => editRow(index)} className="mr">
      ...
      <form id="form" className="budget">
        ...
        <div id="budget">
          <div className="budget-menu">
            <div>
              <button className="btn2" onClick={addBudget}>
                <i className="bx bx-book-add"></i> Add budget
              </button>
            </div>
            <div>
              <label>Remaining Budget</label>
              <h1 className={currentBalance < 0 ? "danger" : ""}>
                {formatNumber(currentBalance)}
              </h1>
            </div>
          </div>

          <div className="budget-body">{budget}</div>
        </div>
      </form>
      {modal}
    </div>
